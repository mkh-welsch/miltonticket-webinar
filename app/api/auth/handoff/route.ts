...
export async function GET(request: NextRequest) {
  try {
    ...
    const remaining = Number(sessionIdentity.handoffExp || 0) - Math.floor(Date.now() / 1000);
    if (remaining < 30) throw new Error("Das Webinar-Handoff läuft ab.");
    response.cookies.set(WEBINAR_SESSION_COOKIE, createWebinarSessionToken(sessionIdentity, remaining), {
      httpOnly: true,
      sameSite: "lax",
